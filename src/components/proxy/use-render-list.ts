  ...
  const renderList = useMemo<IRenderItem[]>(() => {
    ...
    const retList = renderGroups.flatMap((group) => {
      ...
      const filteredMembers =
        regexRule.hasRule && regexRule.isValid
          ? group.members.filter((member) => regexRule.matcher(member.name))
          : group.members
      const displayGroup =
        filteredMembers === group.members
          ? group
      ...
      if (headState.open || !useRule) {
        const occurrences = filterSort(
          resolveOccurrences(proxyView, group).filter(
            ({ member }) =>
              filteredMembers === group.members ||
